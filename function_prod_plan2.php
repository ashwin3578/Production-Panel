...
function check_stock_order($db){
    ...
    HasSub
    from BOM_Detailled
    LEFT JOIN( SELECT [Quantity] as Stock_LEVEL,[Code]as Product_code,TheLocation FROM Current_Stock WHERE 1=1
        )as temp on temp.Product_code=BOM_Detailled.Component
    where  (
        Component not LIKE \'%LABOUR%\' and Component not LIKE \'%STAMPING%\'
    ) and (Component not like \'%MACHINING%\' and Component not like \'%LABEL%\' and Component not like \'%GREASE%\' and Component not like \'%ZSTAMPING%\') and TreeLevel=1
    ...
    foreach($stocklist as $line){
        ...
        $allstock[$line['TheLocation']][$line['Parent']]['List'][]=$line['Component'];
        ...
        //[Quantity]
        if(empty($allstock[$line['Location']][$line['Parent']]['Maximum'])){
