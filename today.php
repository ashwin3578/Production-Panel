...
<div class="container">
	...
	if(!empty($_POST['view'])){
		...
		if (empty($_SESSION['temp']['summary']['view'])){
			$_SESSION['temp']['summary']['view']='Normal';
	...
	// }
	 ...
	 echo'<div class="row" >';
		echo'<div class="col-sm-3">';
			echo'<div class="row" >';
			echo'<Center><h3>List of Scan Still Open - Today</h3></Center>';
			echo'</div>';
			show_list_still_open($db,'yes');
		echo'</div>';
		...
		if(!empty($_SESSION['temp']['addscan']['operator'])){
			...
			// echo'<div class="row " ">';
				...
				load_scan_day($db,$_SESSION['temp']['summary']['operator'],$_SESSION['temp']['summary']['days']);
			 echo'</div>';
