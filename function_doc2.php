...
function manage_POST_document($db){
    ...
    if($_POST['action']=='show_where_doc_is_used'){
        show_where_doc_is_used_details($db,$_POST['doc_number']);
    }
...
function show_doc($db,$workarea='All',$family='All'){
    ...
    function show_wheredocisused(e,doc_number){
        ...
        var request =$.ajax({
            ...
            data: {doc_number:doc_number,action:'show_where_doc_is_used'},
    ...
    foreach ($row as &$logentry){?>
        <div class="row row_document" >
            ...
            <div class="col-sm-1">
            <?php
            if(!empty($allMDS[$logentry['Product_Code']])) {
                foreach($allMDS[$logentry['Product_Code']] as $MDS){
                    //show($MDS);
                    ...
                    onmouseover="show_wheredocisused(this,'<?php echo $MDS['name']?>');"
                    ><?php echo $caption?></a>
                    <?php
                }
            }  
            ...
            <?php
            if(!empty($allWI[$logentry['Product_Code']])) {
                foreach($allWI[$logentry['Product_Code']] as $WI){
                    //show($MDS);
                    $caption=''.$WI['name'];
                    $path=$WI['path'];
                    ?>
                    ...
                    href="<?php echo $path?>"
                    onmouseout="hide_wheredocisused();" 
                    onmouseover="show_wheredocisused(this,'<?php echo $WI['name']?>');"
                    ><?php echo $caption?></a>
                    <?php
                }
            }  
...
function show_where_doc_is_used_details($db,$doc_number){
    $allproduct=get_where_doc_is_used($db,$doc_number);
    //show($allproduct);
    ?>
    <div id="window_where_doc_header" class="window_where_doc_header"><?php echo $type ?>Also used on :</div>
    <br>
        <div class="row">
            <?php foreach($allproduct as $product){?>
                <div class="row"><?php echo$product['doclink_productcode'];?></div>
            <?php } ?>
        </div>
    <?php
}
...
function get_where_doc_is_used($db,$doc_number){
    
    
    $query="SELECT *
    FROM dbo.doc_link
	WHERE doclink_docnumber='$doc_number'";
...
function get_all_MDS($db){
    ...
    //show($row);
    foreach($row as $mds){
        $return[$mds['doclink_productcode']][$mds['doclink_docnumber']]['name']=$mds['doclink_docnumber'];
        $return[$mds['doclink_productcode']][$mds['doclink_docnumber']]['path']='/ressource_v2/MD&S/'.$mds['document_filename'];
...
function get_all_WI($db){
    ...
    left join (SELECT [document_number],
        ...
        FROM dbo.document
        group by document_number,document_filename)as temp on temp.document_number=doclink_docnumber
    WHERE doclink_doctype='Work Instruction'
    ";
    ...
    foreach($row as $mds){
        $return[$mds['doclink_productcode']][$mds['doclink_docnumber']]['name']=$mds['doclink_docnumber'];
        $return[$mds['doclink_productcode']][$mds['doclink_docnumber']]['path']='/ressource_v2/Work Instruction/'.$mds['document_filename'];
