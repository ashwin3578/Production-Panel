...
function manage_POST_prodplan($db){
    show_debug();
    if(empty($_SESSION['temp']['prodplan']['view'])){
        $_SESSION['temp']['prodplan']['view']='normal';
    }
    ...
    if(!empty( $_POST['view'])){
        $_SESSION['temp']['prodplan']['view']=$_POST['view'];
     }
    ...
    $_POST['view']=$_SESSION['temp']['prodplan']['view'];
...
function main_view($db){
    if($_POST['view']=='summary-view'){
		show_summary($db);
	}elseif($_POST['view']=='check-stock'){
		$orderlist=get_all_orders_for_stock($db);
        
        show_stock_all($db,$orderlist);
	}else{
		$orderlist=get_all_orders($db);
		show_order($db,$orderlist);
	}
	...
	echo'<div class="row">';
        ...
        if($_POST['view']=='normal'){
            ...
            echo'<div class="col-sm-1 " >';
                ...
                echo'<button type="submit" name="view" value="check-stock"  class="btn btn-default" >Check Stock</button>';
            ...
            echo'<div class="col-sm-1 " >';
                echo'<button type="submit" name="view" value="normal"  class="btn btn-default" >Back</button>';
        ...
        echo'<div class="col-sm-1 ">';
        if($_POST['view']<>'summary-view'){
            echo'<button type="submit" name="view" value="summary-view"  class="btn btn-default" >Summary View</button>';
        }else{
            echo'<button type="submit" name="view" value="normal"   class="btn btn-default" >Back</button>';
        }
        echo'</div>';
...
function show_order($db,$orderlist){
    ...
    $allstock=check_stock_order_v2($db);
    //show($allstock);
    echo'<style>.Cutting{
            background:#9bb691;
        }
    ...
    foreach($orderlist as $order){
       ...
       //
            echo'<div class="col-sm-4 " >';
                ...
                if($order['QTY_MADE']+$order['QTY_INPROGRESS']>0){echo'onClick="Details'.$order['ManufactureOrderNumber'].'();"';}
                echo'>';
                    echo'<div class="col-lg-10 col-md-8 col-sm-6 " >';
                    echo $order['Code'];
                    echo'</div>';
                    echo'<div class="col-lg-2 col-md-4 col-sm-6  ">';
                    $remaining=$order['BaseQuantityOrdered']-$order['QTY_MADE'];//-$order['QTY_INPROGRESS'];
                    if(($remaining)>$allstock[$order['Location']][$order['Code']]['Maximum']){
                        echo' <div class="popover__wrapper3">';
                        
                        //check if it is a purchased item
                        $warning=0;
                        foreach($allstock['BOM'][$order['Code']]['List'] as $component){
                            if($remaining>$allstock[$order['Location']][$order['Code']][$component]['Equivalent']){
                                if(empty($allstock[$order['Location']][$order['Code']][$component]['HasSub'])){
                                    $warning=1;
                                    echo'<div class="col-sm-6" "><img  src="img/warning.png" width="100%"></div> ';
                                    echo'<div class="col-sm-6" "><img src="img/warning.png" width="100%"></div> ';
                                }
                            }
                        }
                        if (empty($warning)){
                            echo'<div class="col-sm-12" "><img class="" src="img/warning.png" width="50%"></div> ';
                        }
                        echo'<div class="popover__content2 details-qty" style="width:500px;">';
                            echo'<div class="row " style="text-align:center" ><b>Not Enough Stock</b></div>';
                            foreach($allstock['BOM'][$order['Code']]['List'] as $component){
                                ...
                                echo'<div class="row " >';
                                    ...
                                    if($remaining>$allstock[$order['Location']][$order['Code']][$component]['Equivalent']){
                                        echo'<img class="attachment" src="img/warning.png" width="25">';
                                    }
                                    echo'</div>';
                                    echo'<div class="col-xs-5" >'.$component.'</div>';
                                    echo'<div class="col-xs-3" >'.number_format($allstock[$order['Location']][$order['Code']][$component]['Equivalent']).'</div>';
                                    echo'<div class="col-xs-2" >';
                                    if($remaining>$allstock[$order['Location']][$order['Code']][$component]['Equivalent']){
                                        echo'<img class="attachment" src="img/warning.png" width="25">';
                                    }
                    ...
                    echo'</div>';
                    //echo ' '.number_format($allstock[$order['Location']][$order['Code']]['Maximum']);
                    //show($allstock[$order['Location']][$order['Code']]);
...
function show_stock_all($db,$orderlist){
    
    $i=0;
    create_css ($db,'allocationwork');
    $allstock=check_stock_order($db);
    $currentstock=check_current_stock($db);
    //show($orderlist);
    $need=array();
    foreach($orderlist as $location){
       foreach($location as $order){
            $need[$order['Location']][$order['Code']]['Location']=$order['Location'];
            $need[$order['Location']][$order['Code']]['Code']=$order['Code'];
            $need[$order['Location']][$order['Code']]['Type']=$order['Type'];
            $need[$order['Location']][$order['Code']]['QTY_needed']=$need[$order['Location']][$order['Code']]['QTY_needed']+$order['QTY_Remaining'];
    ...
    //show($allstock);
    foreach($need as $location){
        foreach($location as $Code){
            //Show($Code);
            //show($allstock[$Code['Location']][$Code['Code']]);
            foreach($allstock[$Code['Location']][$Code['Code']]['List'] as $component){
                $component_need[$Code['Location']][$component]['Component']=$component;
                $component_need[$Code['Location']][$component]['Location']=$Code['Location'];
                $component_need[$Code['Location']][$component]['HasSub']=$allstock[$Code['Location']][$Code['Code']][$component]['HasSub'];
                $component_need[$Code['Location']][$component]['Qty_required']=$component_need[$Code['Location']][$component]['Qty_required']+$Code['QTY_needed']*$allstock[$Code['Location']][$Code['Code']][$component]['Quantity'];
    ...
    echo'<div class="row header-prod-plan" >';
            echo'<div class="row" >';
                echo'<div class="col-sm-2 " >Code</div>';
                echo'<div class="col-sm-1 " >Stock</div>';
                echo'<div class="col-sm-1 " >Qty Required</div>';
                echo'<div class="col-sm-1 " >Qty Planned</div>';
                echo'<div class="col-sm-2 " >Balance</div>';
                //echo'<div class="col-sm-4 " >'.show($allstock[$order['Location']][$order['Code']]).'</div>';
            echo'</div>';
        echo'</div>';

    foreach($component_need as $location){
        foreach($location as $component){
            //show($component);
            $balance=$currentstock[$component['Location']][$component['Component']]['Stock_LEVEL']-$component['Qty_required']+$need[$component['Location']][$component['Component']]['QTY_needed'];
                
            if ($balance<0){
            echo'<div class="row row-prod-plan row_normal" >';
                echo'<div class="col-sm-2 " >'.$component['Component'].'</div>';
                echo'<div class="col-sm-1 " >'.number_format($currentstock[$component['Location']][$component['Component']]['Stock_LEVEL']).'</div>';
                echo'<div class="col-sm-1 " >'.number_format($component['Qty_required']).'</div>';
                echo'<div class="col-sm-1 " >'.number_format($need[$component['Location']][$component['Component']]['QTY_needed']).'</div>';
                echo'<div class="col-sm-2 " >';
                
                    echo'<div class="col-sm-3 " >';
                    if ($balance<0){echo'<img  src="img/warning.png" width="50%">';}
                    echo'</div>';
                    echo'<div class="col-sm-6 " >';
                ...
                echo'<div class="col-sm-2 " >';
                if ($balance<0){
                    if(empty($component['HasSub'])){
                        echo'Purchased Good';
                    }else{
                        echo'Manufactured Good';
                    }
...
function get_all_orders_for_stock($db){
    $filter=make_filter('simple');
    $pre_sort='';
    ...
    $query='SELECT Code,
    Location,
    sum((BaseQuantityOrdered-COALESCE(QTY_MADE,0)-COALESCE(QTY_INPROGRESS,0))) as QTY_Remaining
	
    FROM MO_List
	
    where  '.$filter.' and ((BaseQuantityOrdered-COALESCE(QTY_MADE,0)-COALESCE(QTY_INPROGRESS,0)))>0
    ...
    $sql->execute();
    //show($query);
    $productlist=$sql->fetchall();
    foreach($productlist as $product){
        $return[$product['Location']][$product['Code']]['Code']=$product['Code'];
        $return[$product['Location']][$product['Code']]['Location']=$product['Location'];
        $return[$product['Location']][$product['Code']]['QTY_Remaining']=$product['QTY_Remaining'];
    }
    //show($productlist);
    return $return;
}

function check_stock_order($db){
    $query="SELECT
    Parent,
    [Component],
    TheLocation,
    ...
    HasSub
    from BOM_Detailled
    LEFT JOIN( SELECT [Quantity] as Stock_LEVEL,[Code]as Product_code,TheLocation FROM Current_Stock WHERE 1=1 and theLocation='QLD'
        )as temp on temp.Product_code=BOM_Detailled.Component 
    where  (
        Component not LIKE '%LABOUR%' and Component not LIKE '%STAMPING%' and Component not LIKE '%ZINDIRECT-OVERHEAD%'
    ) and Parent not like '%CLA-%' and (Component not like '%MACHINING%' and Component not like '%LABEL%' and Component not like '%GREASE%' and Component not like '%ZSTAMPING%') and TreeLevel=1
    
    group by Parent,[Component],HasSub,TheLocation
    order by Parent,round(avg(Stock_LEVEL)/sum([Quantity]),0) DESC";
    $sql = $db->prepare($query); 
    $sql->execute();
    //show($query);
    $stocklist=$sql->fetchall();
    foreach($stocklist as $line){
        $line['TheLocation']='QLD';
        $allstock[$line['TheLocation']][$line['Parent']]['Parent']=$line['Parent'];
        $allstock['BOM'][$line['Parent']]['List'][$line['Component']]=$line['Component'];
        if(!isset($allstock[$line['Location']][$line['Parent']]['Maximum'])){
            $allstock[$line['TheLocation']][$line['Parent']]['Maximum']=$line['Equivalent'];
        }else{
            $allstock[$line['TheLocation']][$line['Parent']]['Maximum']=min($line['Equivalent'],$allstock[$line['TheLocation']][$line['Parent']]['Maximum']);
        }
        $allstock[$line['TheLocation']][$line['Parent']][$line['Component']]['Component']=$line['Component'];
        $allstock[$line['TheLocation']][$line['Parent']][$line['Component']]['Equivalent']=$line['Equivalent'];
        $allstock[$line['TheLocation']][$line['Parent']][$line['Component']]['HasSub']=$line['HasSub'];
        $allstock[$line['TheLocation']][$line['Parent']][$line['Component']]['Quantity']=$line['Quantity'];
        $allstock[$line['TheLocation']][$line['Parent']][$line['Component']]['Stock_LEVEL']=$line['Stock_LEVEL'];
        
        
    }
    //if($_SESSION['temp']['id']=='CorentinHillion'){show($allstock);}
    return $allstock;
}
function check_stock_order_v2($db){
    $query="SELECT
    Parent,
    [Component],
    TheLocation,
    ...
    where  (
        Component not LIKE '%LABOUR%' and Component not LIKE '%STAMPING%' and Component not LIKE '%ZINDIRECT%'
    ) and Parent not like '%CLA-%' and (Component not like '%MACHINING%' and Component not like '%LABEL%' and Component not like '%GREASE%' and Component not like '%ZSTAMPING%' and Component not like '%OVERHEAD%') and TreeLevel=1
...
function check_current_stock($db){
    $query='SELECT [Quantity] as Stock_LEVEL,[Code]as Code,TheLocation FROM Current_Stock';
    $sql = $db->prepare($query); 
    $sql->execute();
    //show($query);
    $stocklist=$sql->fetchall();
    foreach($stocklist as $line){
        $allstock[$line['TheLocation']][$line['Code']]['Code']=$line['Code'];
        $allstock[$line['TheLocation']][$line['Code']]['Stock_LEVEL']=$line['Stock_LEVEL'];
        $allstock[$line['TheLocation']][$line['Code']]['Location']=$line['TheLocation'];
...
function get_data_summary_orders($db,$date){
    ...
    $query="SET DATEFIRST 1;SELECT baseallocationwork_code,
